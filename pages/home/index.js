		...
		if (document.readyState === 'interactive') {
			...
			(() => {
			  ...
			  if (el.origin) {
			    ...
			    if (!ORIGIN_CLASSES.some(c => el.subs.classList.contains(c))) el.subs.classList.add('is--below');
			    const apply = (raw) => { const v = String(raw||'').trim().toLowerCase();
			      removeAll(el.subs, ORIGIN_CLASSES); el.subs.classList.add(ORIGIN_MAP[v] || 'is--below'); };
			  ...
			  if (el.font) {
			    ...
			    const apply = (raw) => { const v = String(raw||'').trim().toLowerCase();
			      removeAll(el.subs, FONT_CLASSES); el.subs.classList.add(FONT_MAP[v] || 'is--poppins'); };
			    apply(el.font.value);
			  ...
			  if (el.color) {
			    ...
			    const apply = (raw) => { const v = String(raw||'').trim().toLowerCase();
			      removeAll(el.subs, COLOR_CLASSES); el.subs.classList.add(COLOR_MAP[v] || 'is--white'); };
